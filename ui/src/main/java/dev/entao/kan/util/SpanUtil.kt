...
import dev.entao.kan.appbase.sp
...
class SpanUtil(s: String) {
    ...
    fun size(sizeSp: Int, start: Int, end: Int): SpanUtil {
        ss.setSpan(
            AbsoluteSizeSpan(sizeSp.sp), start, end,
    ...
    fun imageBaseline(d: Drawable, start: Int, end: Int): SpanUtil {
        ss.setSpan(
            ImageSpan(d, ImageSpan.ALIGN_BASELINE), start, end,
            Spanned.SPAN_INCLUSIVE_EXCLUSIVE
        )
        return this
    }
    ...
    fun roundBackground(backColor: Int, textColor: Int, radius: Int, start: Int, end: Int): SpanUtil {
        ss.setSpan(
            dev.entao.kan.util.RoundBackgroundSpan(backColor, textColor, radius), start, end,
            Spanned.SPAN_INCLUSIVE_EXCLUSIVE
        )
        return this
    }
